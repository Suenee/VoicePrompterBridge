...
export class Logger {
 ...
 private writeTraffic(line:string):void{console.log(line);if(!this.config.enabled)return;try{fs.appendFileSync(this.logFile,`${line}\n`,'utf8')}catch(error){this.debug('Traffic log write failed',error);try{console.error(`Traffic log write failed: ${error instanceof Error?(error.stack??error.message):String(error)}`)}catch{}}}
 ...
 private cleanup():void{try{if(!fs.existsSync(this.logFile))return;const cutoff=Date.now()-this.config.retentionMinutes*60000;const lines=fs.readFileSync(this.logFile,'utf8').split(/\r?\n/);const kept:string[]=[];for(const line of lines){if(!line)continue;const m=line.match(/^(\d{2})\.(\d{2})\.(\d{4}) (\d{2}):(\d{2}):(\d{2})\.(\d{3})/);if(!m)continue;const[,dd,mm,yyyy,hh,min,ss,ms]=m;const ts=new Date(Number(yyyy),Number(mm)-1,Number(dd),Number(hh),Number(min),Number(ss),Number(ms)).getTime();if(ts>=cutoff)kept.push(line)}fs.writeFileSync(this.logFile,kept.length?`${kept.join('\n')}\n`:'','utf8')}catch(error){this.debug('Traffic log cleanup failed',error);try{console.error(`Traffic log cleanup failed: ${error instanceof Error?(error.stack??error.message):String(error)}`)}catch{}}}
}
