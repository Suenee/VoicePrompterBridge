...
internal sealed class SUBContext:ApplicationContext
{
 ...
 void ShowMailboxes(){if(mailboxes!=null&&!mailboxes.IsDisposed){RestoreWindow(mailboxes);return;}mailboxes=new MailboxesForm(Db,Status,icon);mailboxes.FormClosed+=(_,_)=>mailboxes=null;mailboxes.Show();mailboxes.Activate();}
