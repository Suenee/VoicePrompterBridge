...
internal sealed class SUBContext:ApplicationContext
{
 ...
 void WriteDiagnostic(string kind,string detail){try{string line=$"{DateTime.Now:yyyy-MM-dd HH:mm:ss.fff} SYSTEM {kind} {detail.Replace("\r"," ").Replace("\n"," | ")}";lock(diagnosticLogLock)File.AppendAllText(Log,line+Environment.NewLine);}catch{}}
 void CaptureServerOutput(string stream,string? data){if(string.IsNullOrWhiteSpace(data))return;WriteDiagnostic($"SERVER {stream}",data);}
 ...
 void Start(){if(server is{HasExited:false}){SetState(BridgeState.Running);return;}try{EnsureNode();if(!File.Exists(Script))throw new FileNotFoundException("Missing dist\\main.js. Run npm run build first.");if(!File.Exists(Config))throw new FileNotFoundException("Missing config\\vpbridge.json.");server=Process.Start(new ProcessStartInfo{FileName=ServerExe,Arguments=$"\"{Script}\"",WorkingDirectory=baseDir,UseShellExecute=false,CreateNoWindow=true,WindowStyle=ProcessWindowStyle.Hidden,RedirectStandardInput=true,RedirectStandardOutput=true,RedirectStandardError=true})??throw new InvalidOperationException("Server process could not be started.");server.StandardInput.AutoFlush=true;server.OutputDataReceived+=(_,e)=>CaptureServerOutput("STDOUT",e.Data);server.ErrorDataReceived+=(_,e)=>CaptureServerOutput("STDERR",e.Data);server.BeginOutputReadLine();server.BeginErrorReadLine();Thread.Sleep(300);if(server.HasExited){int code=server.ExitCode;WriteDiagnostic("SERVER EXIT",$"Server stopped immediately after start. exitCode={code}");server.Dispose();server=null;throw new InvalidOperationException($"Server stopped immediately after start. Exit code: {code}.");}SetState(BridgeState.Running);}catch(Exception e){WriteDiagnostic("SERVER START ERROR",e.ToString());server?.Dispose();server=null;SetState(BridgeState.Error,e.Message);tray.ShowBalloonTip(5000,"SUB ERROR",e.Message,ToolTipIcon.Error);}}
...
internal static class SUBProgram
{
 ...
 static string CrashLog=>Path.Combine(AppContext.BaseDirectory,"logs","SocketUniverseBridge.log");
 static void LogUnhandled(string kind,Exception? e){try{Directory.CreateDirectory(Path.GetDirectoryName(CrashLog)!);File.AppendAllText(CrashLog,$"{DateTime.Now:yyyy-MM-dd HH:mm:ss.fff} SYSTEM {kind} {(e?.ToString()??"Unknown exception").Replace("\r"," ").Replace("\n"," | ")}{Environment.NewLine}");}catch{}}
